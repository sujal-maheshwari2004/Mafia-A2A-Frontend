    ...
    <div className="mt-4 rounded-xl border border-amber-400/15 bg-amber-400/[0.04] p-3.5">
      <div className="mb-2.5 flex items-center justify-between">
        <h3 className="text-[11px] font-medium uppercase tracking-wider text-amber-200/70">
          ⚖️ The room turns on someone — live
        </h3>
        <span className="text-[11px] tabular-nums text-stone-300/35">
          {votes.length} of {voterCount} cast
        </span>
      </div>

      {standings.length === 0 ? (
        <p className="text-xs italic text-stone-300/35">Votes are landing — no one's named anyone yet…</p>
      ) : (
        <div className="flex flex-col gap-1.5">
          {standings.map(([name, count]) => (
            <div key={name} className="flex items-center gap-2">
              <span className="w-[4.5rem] shrink-0 truncate text-xs text-stone-300/70">{name}</span>
              <div className="h-2 flex-1 overflow-hidden rounded-full bg-black/25">
                <div
                  className={`h-full rounded-full transition-all duration-700 ease-out ${
                    count === topCount ? 'bg-red-600/75' : 'bg-amber-400/40'
                  }`}
                  style={{ width: `${(count / scale) * 100}%` }}
                />
              </div>
              <span className="w-4 shrink-0 text-right text-xs font-semibold tabular-nums text-amber-50/80">
                {count}
              </span>
            </div>
          ))}
      ...
      <ol className="mt-3 flex flex-wrap gap-x-3 gap-y-1 border-t border-amber-100/[0.06] pt-2.5 text-[11px] text-stone-300/35">
        {votes.map((vote, i) => (
          <li key={i} className="flex items-center gap-1 animate-[fadeIn_0.4s_ease-out]">
            <span className="text-stone-300/60">{vote.voter}</span>
            <span className="text-stone-400/25">→</span>
            <span className={vote.target ? 'text-amber-200/70' : 'italic text-stone-300/35'}>
